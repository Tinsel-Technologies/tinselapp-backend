...
-----END CERTIFICATE-----`;
    ...
    this.initiatorPassword = 'Tin@105117';

    if (!this.publicKey) {
      this.logger.error('MPESA_PUBLIC_CERT is not configured!');
      throw new Error('Public certificate is missing');
    }

    if (!this.initiatorPassword) {
      this.logger.error('MPESA_INITIATOR_PASSWORD is not configured!');
      throw new Error('Initiator password is missing');
    }

    this.logger.log(
      'MpesaSecurityService initialized with hardcoded credentials',
    );
  }
